...
def work():
                ...
                if add == 1:
                    ...
                    mycursor.execute(b)
                    for i in mycursor:
                        print("Train Name","Train No.","Journey From","Destination")
                        print(i)
                    mysql.commit()
...
def reservation():
                ...
                l1.append(pnr)
                train1=(l1)
                cr = "create table if not exists passengers(pname varchar(183),age varchar(183),trainno varchar(183),noofpas varchar(183),cls varchar(183),amt varchar(183),status varchar(183),pnrno varchar(183))"
                ...
                print("Train Name,Train No.,Journey From,Destination,Passenger name ,Age,Train No.,No of Passenger,Class,Amount,Status,PNR No")
                for y in mycursor:
                    print(y)
...
def displayPNR():
                ...
                a = ("(Passenger name,age,train no, no of passenger,class,amount,status, PNR)")
                for x in res:
                                print(x)   
                #print("Deletion completed")
                print("Go back to menu OR Skip ")
...
def users():
            ...
            if OPTION == "1":
                ...
                det = [Name,Age,Sex,Mob]
                print("Please set a password of mininmum 6 digits : ")
                pswad = input("")
                det.append(pswad)
                deta = (det)
                w = "create table if not exists user(Name varchar(183),Age varchar(183),Sex varchar(5),Mob varchar(183),Password varchar(183))"
                w1 = "insert into user(Name,Age,Sex,Mob,Password) values(%s,%s,%s,%s,%s)"
                mycursor.execute(w)
